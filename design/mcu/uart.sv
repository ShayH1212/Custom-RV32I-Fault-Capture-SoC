...
4. UART Idle State is HIGH
   The TX line remains HIGH while no data is being transmitted.
   A LOW signal indicates the beginning of the START bit.
...
6. Single Byte TX Storage
   One byte is stored in tx_data_register while it is being transmitted.
   There is no transmit FIFO in this version.

7. TX Writes Only Accepted While Idle
   The CPU can start a new transmission only while the TX FSM is in TX_IDLE.
   Writes that occur while a transmission is active are not accepted.

8. Single Byte RX Storage
   Received bits are first collected in rx_shift_register and then stored
   in rx_data_register when a complete valid byte has been received

9. Two Flip Flop RX Synchronizer
   The external RX signal passes through two flip flops rx_unstable and rx_synced before
   reaching the receiver FSM to reduce metastability risk.

10. Mid Bit START Confirmation
...
11. Single Sample Per Data Bit
    After the START bit is confirmed, RX is sampled once every baud period.
    Oversampling is not used in this version.

12. RX Valid Flag
    rx_valid becomes HIGH when a complete valid byte has been received.
    It remains HIGH until the CPU clears it.

13. Write 1 to Clear RX Valid
    Writing a 1 to bit 1 of UART_STATUS clears rx_valid.
    Reading UART_DATA does not automatically clear the flag.


14. No UART Interrupt
    This UART does not currently generate an interrupt signal.
    The CPU determines UART status by reading UART_STATUS.
...
Register Map:
    ...
    0x0 = UART_DATA
          ...
          Writing:
          Stores the byte that will be transmitted.
    ...
    0x4 = UART_STATUS
          ...
          Writing a 1 to bit 1 clears RX valid.
...
The reciever also uses an FSM as defined below 
    ...
    RX_IDLE
    Waits for the RX line to go low, which may indicate the beginning of a
    start bit.

    RX_START
    Waits until the middle of the start bit and checks that the RX line is
    still low.

    RX_DATA
    Samples the RX line once per baud period and stores each of the 8 received
    data bits.

    RX_STOP
    Checks for a valid stop bit, which should be a 1. If valid, the
    received byte is stored and marked as ready for the CPU.
    ...
    if (reset) begin
        ...
        if (rx_state == RX_IDLE) begin
            ...
            if (rx_baud_counter == (BAUD_DIVIDER / 2)) begin
                ...
                // Confirm that the line is still low
                if (rx_synced == 1'b0) begin
